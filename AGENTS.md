...
Fabric mod (Minecraft 1.20.1, server-side only) that monitors world eaters, trenchers, and bedrock breakers, sending Discord webhook notifications with per-event ping control when they stop or get obstructed.

## Tech Stack

- **Language:** Java 17
- **Framework:** Fabric Loader 0.15.11, Fabric API 0.92.2+1.20.1
- **Build:** Gradle (Fabric Loom 1.7-SNAPSHOT)
- **Mappings:** Yarn 1.20.1+build.10
- **Dependencies:** JDA 5.2.1 (shaded) + stdlib (java.net.http for Discord webhooks, Gson for config)
- **Mixin:** ExplosionMixin targeting `Explosion.affectWorld`
...
2. **Block-break-based (Trencher, BedrockBreaker):** `ExplosionMixin` hooks `Explosion.affectWorld` — before destruction it captures the affected block states (`getAffectedBlocks`), after it filters out air/TNT and fires `ExplosionBlockCallback.EVENT` with the list of actually-destroyed positions. `MonitorCheckHandler.onExplosionBlocksDestroyed` counts how many fall inside each active machine's AABB. If count >= `minBlocksBroken`, updates activity tick.
