...
Fabric mod (Minecraft 1.21.6, server-side only) that monitors world eaters, trenchers, and bedrock breakers, sending Discord webhook or bot notifications with per-event ping control when they stop or get obstructed.

## Tech Stack

- **Language:** Java 21
- **Framework:** Fabric Loader 0.16.14, Fabric API 0.128.0+1.21.6
- **Build:** Gradle (Fabric Loom 1.10.5)
- **Mappings:** Yarn 1.21.6+build.1
- **Dependencies:** Fabric API, JDA 5.2.1 (shaded into jar for bot mode), stdlib (java.net.http for Discord webhooks, Gson for config)
...
│   ├── DiscordNotifier.java            # Dual routing: webhook HTTP or JDA bot
...
- Op players (permission level 2+) can always use commands.
...
<command> settings setBotToken <token>
<command> settings setGuildId <id>
<command> settings setChannelId <id>
<command> settings setMemberDiscordRole <roleId>
<command> settings setNotificationMode webhook|bot
...
<command> settings showSubscriptionButton <true|false>
