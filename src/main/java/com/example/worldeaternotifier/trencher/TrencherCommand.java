...
public class TrencherCommand {
    ...
    private static int executeStart(CommandContext<ServerCommandSource> ctx) {
        ...
        if (!isDeliveryConfigured()) {
            ctx.getSource().sendError(Text.literal("Cannot start: notification delivery not configured. Use /trencher settings setWebhookUrl or setBotToken/setGuildId/setChannelId first."));
            return 0;
        }
        manager.start(name);
